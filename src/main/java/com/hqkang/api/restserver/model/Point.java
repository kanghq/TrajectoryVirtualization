...
public class Point  implements Serializable, Comparable{
	...
	public com.vividsolutions.jts.geom.Point toJTSPoint(boolean GCJ) {
		Double longitude,latitude;
		if(false == GCJ) {
			longitude = this.longitude;
			latitude = this.latitude;
		} else {
			Double lonlat[] = PointConv.getInstance().convert(this.longitude, this.latitude);
			longitude = lonlat[0];
			latitude = lonlat[1];
		...
		Coordinate coo = new Coordinate(longitude, latitude);
