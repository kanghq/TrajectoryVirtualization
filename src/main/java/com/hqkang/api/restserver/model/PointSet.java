...
public class PointSet extends LinkedHashSet<Point> implements Serializable{
	...
	public LineString getLineString(boolean gcj) {
		
		GeometryFactory fact = new GeometryFactory();

		Coordinate[] list = new Coordinate[this.tSet.size()<2?2:this.tSet.size()];
		int i = 0;
		for(Point pt: tSet.values()) {
			list[i] = new Coordinate(pt.toJTSPoint(gcj).getCoordinate());
			i++;
		}
		
		if(this.tSet.size()<2) {
			list[1] = new Coordinate(tSet.values().iterator().next().toJTSPoint(gcj).getCoordinate());
		}
		
		return fact.createLineString(list);
	}
